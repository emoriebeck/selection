...
sca_plot_fun <- function(d1, type, trait, outcome){
  ...
  file <- sprintf("%s/results/sca/plots/%s/sca_%s_%s.pdf", wd, type, outcome, trait)
...
sca_compile_fun <- function(trait, outcome, d){
  ...
  save(results,  file = sprintf("%s/results/sca/inf_test/%s_%s.RData", wd, outcome, trait))
...
# done <- tibble(
#   files = sprintf("%s/results/sca/inf_test", wd) %>% list.files()
# ) %>% 
#   separate(files, c("Outcome", "Trait"), sep = "_") %>%
...
read_fun <- function(file){
  load(sprintf("%s/results/sca/inf_test/%s", wd, file))
  return(results)
}
...
sca_res %>% 
  ...
  select(Outcome, Test, all_of(traits$short_name)) %>%
  mutate(Test = mapvalues(Test, c("med_test", "sign_test", "sig"), c("Median", "Sign", "Significance"))) %>%
  mutate_at(vars(-Outcome, -Test), ~ifelse(. < .05, sprintf("<strong>%.2f</strong",.), sprintf("%.2f",.))) %>%
  kable(.
        , "html"
        , escape = F
